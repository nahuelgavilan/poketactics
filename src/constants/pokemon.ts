...
/**
 * Available Pokemon pool for the game
 * Each Pokemon has balanced stats for tactical gameplay
 */
 ...
 * Uses evolution chains - Pokémon start as base forms and can evolve
 */
...
/**
 * Get a random Pokemon from the full pool (for wild captures)
 * These are final evolutions that can be captured
 */
export function getRandomWildPokemon(excludeIds: Set<number> = new Set()): PokemonTemplate {
  const available = POKEMON_POOL.filter(p => !excludeIds.has(p.id));
